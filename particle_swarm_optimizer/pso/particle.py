...
class Particle:
    ...
    def _init_particle(self):
        if self.random is True:
            self.position = np.array([self.random.uniform(self.position_range[0], self.position_range[1]) for _ in range(self.dims)])
            self.velocity = np.array([self.random.uniform(self.velocity_range[0], self.velocity_range[1]) for _ in range(self.dims)])
        elif self.random is False:
